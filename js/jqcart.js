...
;(function($) {
  ...
  var actions = {
    ...
    addToCart: function(e) {
      ...
      if(typeof itemData.id === 'undefined') {
        console.log('Відсутній ID товару');
        return false;
      }
      cartData = actions.getStorage() || {};
    ...
    openCart: function() {
      ...
      cartHtml = '';
      cartData = actions.getStorage();
      orderPreview = '<p class="jqcart-cart-title">Корзина <span class="jqcart-print-order"></span></p><div class="jqcart-table-wrapper"><div class="jqcart-manage-order"><div class="jqcart-thead"><div>ID</div><div></div><div>Найменування</div><div>Ціна</div><div>Кіл-ть</div><div>Сума</div><div></div></div>';
      var key, sum = 0;
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
      ...
      orderPreview += '<div class="jqcart-subtotal">До сплати: <strong>' + subtotal + '</strong> ' + opts.currency + '</div>';
    ...
    printOrder: function (){
      ...
      var win = window.open('', 'Друкування замовлення"><input', 'width='+screen.width+',height='+screen.height),
  ...
  $.jqCart = function(opts) {
    if (methods[opts]) {
      ...
      $.error('Метод з іменем "' + opts + '" не існує!');
    }
  };
})(jQuery);
