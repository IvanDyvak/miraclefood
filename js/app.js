...
// ******************RANDOMIZING TIPS DAILY********************
    ...
    let renderResults = (savedRecipes, page = 1, resPerPage = 2) =>{
        ...
        document.querySelector(".recipe_container").addEventListener('click', e =>{
            const btn = e.target.closest('.btn_inline');
        if (btn) {
            const goToPage = parseInt(btn.dataset.goto, 4);
            // function delItem() {
            //     var ul = document.querySelector('.recipe_container');
            //     while (ul.hasChildNodes()) {
            //         ul.removeChild(ul.firstChild);
            //     }
            // }delItem();
